// src/api/market.ts
// Fixed Day 61 audit: searchStocks uses ?query= (matches backend /api/stocks/search)
...
export const marketApi = {
  ...
  // Fixed: backend controller uses ?query= not ?q=
  searchStocks: (q: string) =>
    apiClient.get(`/api/stocks/search?query=${encodeURIComponent(q)}`).then(r => r.data),
}
